...
async function getGameInfo(slug) {
  try {
    ...
    const short_description = row_description.textContent.slice(0, 160);
...
async function createManyToManyData(products) {
  ...
  products.forEach((product) => {
    ...
    publishers?.forEach((publisher) => plublishersSet.add(publisher));
    operatingSystems?.forEach((platform) => platformsSet.add(platform));
  ...
  return Promise.all([
    ...
    ...createCall(plublishersSet, publisherService),
